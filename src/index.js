...
const salt = bcrypt.genSaltSync(10); //Rondas que utilizamos para
...
//app.use(cors('https://erp.spaceti.cloud/'))
...
if (GEMINI_API_KEY) {
  ...
  console.error(
    "⚠️ Advertencia: GEMINI_API_KEY no encontrada. La funcionalidad de IA estará deshabilitada."
  );
  ai = null;
  model = null; // Asegura que 'model' también sea null
}
...
const functionDeclaration = {
  ...
  description:
    "Obtiene el saldo actual y el estatus de una factura específica utilizando el folio fiscal.",
  parameters: {
    type: "OBJECT",
    properties: {
      tipo_consulta: {
        type: "STRING",
        description:
          "El tipo de consulta a realizar. Puede ser 'Pedimento' para desplegar informacion del mismo, 'Factura', 'Finanzas', 'Pólizas', etc",
      },
      fiscal_folio: {
        type: "STRING",
        description:
          "El folio fiscal o la referencia fiscal de la factura, por ejemplo, 'A 1984', 'AB-123', '15256'.",
      },
    },
  ...
  try {
    ...
    if (result.recordset && result.recordset.length > 0) {
      ...
      return {
        ...
        error_message:
          "La factura fue consultada, pero no se encontró un registro de saldo. Verifique los folios y RFC.",
      };
    }
  } catch (sqlError) {
    // Manejo de errores de conexión o ejecución
    console.error(
      `[SQL ERROR FATAL] Error al ejecutar Consulta_SQL_ChatBot: ${sqlError.message}`
    );
    return {
...
async function generateContentWithRetry(contents) {
  ...
  if (typeof fetch === "undefined") {
    throw new Error(
      "ERROR FATAL: La función 'fetch' no está disponible. Asegúrate de instalar 'node-fetch@2' si usas Node < 18."
    );
  }
  ...
  while (maxCalls > 0) {
    ...
    console.log(
      `[API CALL] Llamada a Gemini (recursividad restante: ${maxCalls})`
    );
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        if (!response.ok) {
          ...
          console.error(
            `[API ERROR] Status: ${response.status}, Body: ${errorBody}`
          );
        ...
        if (attempt === MAX_RETRIES) {
          throw new Error(
            "El servicio de IA falló después de múltiples reintentos."
          );
        }
      }
    }
    ...
    if (!candidate) {
      console.error(
        "[GEMINI FAIL] No se encontró candidato en la respuesta. Respuesta completa:",
    ...
    if (functionCall) {
      ...
      console.log(
        `[FUNCTION CALL DETECTED] Name: ${name}, Args: ${JSON.stringify(args)}`
      );
    ...
    console.error(
      "[GEMINI FAIL] El candidato no contiene texto ni llamada a función. Respuesta:",
      JSON.stringify(responseJson, null, 2)
    );
...
app.post("/api/chat", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        error: error.message || "Error interno al procesar la solicitud de IA.",
      });
  }
});
...
const documentExtractionSchema = {
  ...
  properties: {
    ...
    invoice_date: {
      type: "string",
      description:
        "Fecha de la Factura (Invoice Date) en formato YYYY-MM-DD. Si no es un formato de fecha válido, devuelve 'N/A'.",
    },
    po_number: {
      type: "string",
      description:
        "Número de Orden de Compra (PO Number). Si no existe, devuelve una cadena vacía.",
    }, // --- DATOS DE MONTO Y PESO ---
    currency: {
      type: "string",
      description: "Moneda utilizada (ej. USD, MXN, EUR).",
    },
    subtotal_amount: {
      type: "number",
      description:
        "El monto subtotal de la Factura (antes de impuestos/cargos). Si no existe, devuelve 0.",
    },
    tax_amount: {
      type: "number",
      description:
        "El monto total de impuestos (Tax Amount). Si no existe, devuelve 0.",
    },
    total_amount: {
      type: "number",
      description:
        "El monto total final de la Factura (Invoice Amount) en números.",
    },
    total_gross_weight_kg: {
      type: "number",
      description:
        "El peso bruto total (Gross Weight) en kilogramos (KG). Si solo hay peso en LB, convertirlo a KG (1 lb = 0.453592 kg) y devolverlo aquí.",
    }, // --- INFORMACIÓN DE ENTIDADES ---
    vendor_name: {
      type: "string",
      description:
        "Nombre completo de la entidad que emite la factura (Proveedor/Vendedor).",
    },
    vendor_tax_id: {
      type: "string",
      description: "Número de identificación fiscal o RFC del proveedor.",
    },
    sold_to_id: {
      type: "string",
      description: "El ID de la entidad 'SOLD TO' (ej. P3621 ID:).",
    }, // --- DETALLE DE PRODUCTOS (ARRAY) ---
    products: {
      type: "array",
      description:
        "Lista de todos los productos o servicios detallados en las líneas de la factura. Si no hay líneas de producto, devuelve un array vacío (ej. []).",
      items: {
        type: "object",
        properties: {
          item_sku: {
            type: "string",
            description: "SKU o número de parte del producto.",
          },
          item_description: {
            type: "string",
    ...
    observaciones_ia: {
      type: "string",
      description:
        "Observación sobre la legibilidad del documento (ej. 'Documento escaneado y claro', 'Algunos campos están borrosos').",
    },
...
const documentExtractionSchema_SP = {
  ...
  type: "object", // Las propiedades del objeto
  properties: {
    // --- INFORMACIÓN CLAVE DEL DOCUMENTO ---
    ...
    fecha_factura: {
      type: "string",
      description:
        "Fecha de la Factura en formato YYYY-MM-DD. Si no es un formato de fecha válido, devuelve 'N/A'.",
    },
    numero_orden_compra: {
      type: "string",
      description:
    ...
    metodo_pago: {
      type: "string",
      description: "Método de Pago utilizado en la Factura.",
    },
    forma_pago: {
      type: "string",
      description: "Forma de Pago utilizada en la Factura.",
    },
    tasa_iva: {
      type: "number",
      description: "Tasa de IVA aplicada en la Factura.",
    }, // --- DATOS DE MONTO Y PESO ---
    moneda: {
      type: "string",
      description: "Moneda utilizada (ej. USD, MXN, EUR).",
    ...
    domicilio_emisor: {
      type: "string",
      description: "Domicilio completo del emisor.",
    },
    regimen_fiscal_emisor: {
      type: "string",
      description: "Clave y Regimen fiscal del emisor.",
    },
    nombre_receptor: {
      type: "string",
      description:
        "Nombre completo de la entidad que recibe solicita la factura (cliente).",
    }, // ✅ CORREGIDO: rfc_receptos -> rfc_receptor
    rfc_receptor: { type: "string", description: "RFC del cliente/receptor." },
    domicilio_receptor: {
      type: "string",
      description: "Domicilio completo del receptor.",
    },
    regimen_fiscal_receptor: {
      type: "string",
      description: "Clave y Regimen fiscal del receptor.",
    }, // --- DETALLE DE PRODUCTOS (ARRAY) ---
    productos: {
      type: "array", // ✅ CORRECCIÓN: Se usa 'description' en lugar de 'descripcion'
      description:
        "Lista de todos los productos o servicios detallados en las líneas de la factura. Si no hay líneas de producto, devuelve un array vacío (ej. []).",
      items: {
        type: "object",
        properties: {
          ...
          clave_producto: {
            type: "string",
            description: "Clave del producto o servicio.",
          },
          clave_unidad: {
            type: "string",
            description: "Clave de la unidad de medida.",
          },
          unidad_medida: {
            type: "string",
...
function getMimeTypeFromExtension(tipoArchivo, isText = false) {
  if (isText) {
    if (tipoArchivo && tipoArchivo.toLowerCase().includes("xml"))
      return "application/xml";
    if (tipoArchivo && tipoArchivo.toLowerCase().includes("json"))
      return "application/json";
    return "text/plain";
  }
  if (tipoArchivo && tipoArchivo.toLowerCase().includes("pdf"))
    return "application/pdf";
  if (tipoArchivo && tipoArchivo.toLowerCase().includes("jpg"))
    return "image/jpeg";
  if (tipoArchivo && tipoArchivo.toLowerCase().includes("png"))
    return "image/png";
...
function formatSqlParams(parametros) {
  ...
  console.log(
    "[DEBUG] Raw Input Parameters (FormatSQL):",
  ...
  for (let key in parametros) {
    ...
    if (typeof value === "string" || value instanceof String) {
      ...
      rawStringValue = value.trim(); // Detectar y remover comillas externas innecesarias que pueda añadir la IA
      if (rawStringValue.startsWith("'") && rawStringValue.endsWith("'")) {
        rawStringValue = rawStringValue
          .substring(1, rawStringValue.length - 1)
          .trim();
      }
    } // TRATAMIENTO DE NÚMEROS: Si es un número puro, no lo citamos.

    let cleanedNumericTest =
      typeof rawStringValue === "string"
        ? rawStringValue.replace(/[\s\uFEFF\xA0]+/g, "")
        : rawStringValue; // Solo si la cadena restante es puramente dígitos (sin punto decimal).

    const isPureInteger =
      typeof cleanedNumericTest === "string" &&
      /^\d+$/.test(cleanedNumericTest);

    if (isPureInteger) {
      quotedValue = cleanedNumericTest; // Número sin comillas
    } else if (
      typeof rawStringValue === "string" ||
      rawStringValue instanceof String
    ) {
    ...
    if (stringParametros.length > 0) stringParametros += ", "; // Asumimos que el SP es posicional (sin @nombreParam)
    stringParametros += `${quotedValue}`;
  }
  console.log(
    "[DEBUG] Formatted SQL Parameters (FormatSQL):",
    stringParametros
  );
...
app.get("/", async (req, res) => {
  try {
    await sql.connect(sqlConfig); // Prueba de conexión simple
    const result = await sql.query("SELECT GETDATE() as CurrentDateTime");
    res.send(
      "✅ DB connection successful: " + JSON.stringify(result.recordset)
    );
  } catch (err) {
    console.error(err);
...
app.post("/user/actualiza-password", async (req, res) => {
  ...
  try {
    ...
    const result = await sql.query(
      `UPDATE Personal_Passwords SET password = '${hash}' WHERE usuario = '${usuario}'`
    );
    ...
    return res
      ...
      .json({
        success: false,
        message: "No se pudo actualizar la contraseña para el usuario",
      });
  }
});
...
app.post("/user/login", async (req, res) => {
  ...
  try {
    ...
    if (!usuarioInput) {
      return res
        .status(400)
        .json({ success: false, message: "Usuario is required" });
    }
    ...
    if (
      ...
      result.recordset[0].vigencia !== "Válido"
    ) {
      return res
        .status(401)
...
app.post("/dinamico/lista", async (req, res) => {
  try {
    ...
    if (instruccionSQL == "Trae_Tramite_Aduanal") {
      ...
      console.log("NUMBER FOLIO", numberFolio, typeof numberFolio); //Intentar conectarse a la BD

      await sql.connect(sqlConfig); //Consulta con instruccionSQL y numbero de folio

      const result = await sql.query(`${instruccionSQL} ${numberFolio}`);

      console.log(result.recordsets); //Respuesta enviada
    ...
    console.log(`${instruccionSQL}  ${stringParametros}`); //Intentar conectarse a la BD
...
app.post("/consulta", async (req, res) => {
  ...
  console.log(consulta); // if (!Consulta || !Otros || !Solo_Activas) { //   return res.status(400).json({ error: 'Faltan parámetros requeridos' }); // }

  try {
    await sql.connect(sqlConfig); // Ejemplo: si parametro2 es el nombre del procedimiento y parametro1 es el valor
...
app.post("/ejecuta", async (req, res) => {
  try {
    ...
    //---------------------------------------------------------------------
    // Logica para Rep_Facturas_Tramites_SPACE7 unicamente !!!!!!!!

    // Logica para crear archivo xlsx para el sp Rep_Facturas_Tramites_SPACE7 unicamente
    // if (instruccionSQL == "Rep_Facturas_Tramites_SPACE7") {
    //   // 👇 Create a Request instance and set timeout
    //   const request = new sql.Request();
    //   request.timeout = 240000; // 4 minutes (240,000 ms) — this prevents the 15s SQL timeout!

    //   const result = await request.query(
    //     `${instruccionSQL} ${stringParametros}`
    //   );

    //   // 💡 Critical: data[0] in frontend = result.recordset in backend
    //   const data = result.recordset; // This is your array of rows

    //   if (!data || data.length === 0) {
    //     return res.status(404).json({ error: "No data found" });
    //   }

    //   // ✅ EXACT SAME LOGIC AS YOUR FRONTEND — but in Node.js!
    ...
    //   res.setHeader(
    //     "Content-Disposition",
    //     'attachment; filename="facturas_styled.xlsx"'
    //   );
    //   res.send(buffer);
...
app.post("/analizar-documento-gemini", async (req, res) => {
  console.log("[ENDPOINT] /analizar-documento-gemini iniciado."); // Asegúrate de que 'ai' o el SDK de Google Generative AI esté inicializado
  ...
  try {
    ...
    const { base64, mimeType } = file; // 2. VALIDACIÓN AMPLIADA DE ENTRADAS
    ...
    if (!base64 || !mimeType || !tipoDocumento || !prompt) {
      return res
        ...
        .json({
          success: false,
          message:
            "Faltan datos (file,  tipoDocumento, o prompt) en la solicitud.",
        });
    }
    console.log(
      `[Gemini] Análisis para Trámite ID: ${tramite_id}, Tipo: ${mimeType}`
    );
    ...
    if (isDocumentExtraction) {
      ...
      if (!tipoDocumento) {
        return res.status(400).json({
          success: false,
          message:
            "El tipoDocumento es requerido para la extracción de documentos estructurados.",
        });
      }
...
            `;
      ...
      dynamicPrompt = prompt; // Usar el prompt del usuario directamente // No se necesita generationConfig si la respuesta es texto libre
    }
    ...
    console.error("Error en el análisis multimodal con Gemini:", error.message); // Si la IA falla al generar JSON, el error de parseo será atrapado aquí.
    res.status(500).json({
      success: false,
      message:
        "Error interno en el servicio de IA. Verifique el formato de respuesta esperado.",
    });
  }
});
...
app.post("/documento/obtener", async (req, res) => {
  ...
  console.log(
    `[DEBUG 1.2 - Entrada] parametros: ${JSON.stringify(parametros)}`
  );
  console.log(
    `[DEBUG 1.3 - Entrada] tipoArchivo (MIME Sugerido): ${tipoArchivo}`
  );
  ...
  if (!instruccionSQL) {
    ...
    return res
      ...
      .json({
        success: false,
        message: "La instrucción SQL (SP) es requerida.",
      });
  }
  ...
  try {
    ...
    console.log(
      `[DEBUG 4.1 - Resultado] Total de recordsets devueltos: ${result.recordsets.length}`
    );

    if (!result.recordset || result.recordset.length === 0) {
      console.error(
        "❌ [ERROR 404] El SP se ejecutó pero no devolvió registros."
      );
      return res
        .status(404)
        .json({
    ...
    console.log(
      "[DEBUG 5.1 - Registro] Claves del primer registro:",
      Object.keys(firstRecord)
    );

    const dataKey = Object.keys(firstRecord).find(
      (key) =>
        firstRecord[key] instanceof Buffer ||
        typeof firstRecord[key] === "string"
    );

    console.log(
      `[DEBUG 5.2 - Key] Clave de columna con datos (dataKey) encontrada: ${dataKey}`
    );
    ...
    if (!binaryData) {
      console.error(
        "❌ [ERROR 404] El campo de datos binarios es null o no se encontró una columna válida."
      );
      return res
        .status(404)
        .json({
          success: false,
          message:
            "El procedimiento no devolvió un campo de datos binarios válido.",
        });
    }

    console.log(
      `[DEBUG 5.3 - Tipo Dato] binaryData es una instancia de Buffer: ${
        binaryData instanceof Buffer
      }`
    );
    console.log(
      `[DEBUG 5.4 - Tipo Dato] typeof binaryData: ${typeof binaryData}`
    );
    ...
    if (binaryData instanceof Buffer) {
      ...
      console.log(
        `[DEBUG 6.2.0 - Limpieza] Longitud después de limpieza: ${fileData.length}`
      );
      ...
      const rawExtensionParam =
        parametros["@cExtension"] || parametros["cExtension"] || "";
      const requestedExtension = rawExtensionParam
        .replace(/'/g, "")
        .toUpperCase();
      ...
      if (
        ...
        console.log(
          "[DEBUG 6.2.1 - PDF Fix] MIME Type forzado a application/pdf para el string Base64."
        );
      } else {
        mimeType = getMimeTypeFromExtension(tipoArchivo, true);
        console.log(
          "[DEBUG 6.2.2 - Text Fallback] MIME Type asignado por la función auxiliar."
        );
      }
      // --------------------------------------------------------
    } else {
      ...
      return res
        ...
        .json({
          success: false,
          message: "Tipo de dato binario desconocido devuelto por el SP.",
        });
    }
    ...
    console.error(
      "🔥 [ERROR CRÍTICO] Excepción capturada en /documento/obtener:",
      error
    );
    res.status(500).json({
      success: false,
      message:
        "Error interno del servidor al obtener el documento. Revise el log del servidor para detalles.",
...
app.post("/analisis-ia", async (req, res) => {
  ...
  if (!ai) {
    return res
      ...
      .json({
        success: false,
        message: "API de Gemini no inicializada. Verifique GEMINI_API_KEY.",
      });
  }
  ...
  try {
    ...
    if (!instruccionSQL || !promptAI) {
      return res
        .status(400)
        .json({ error: "Instrucción SQL y promptAI son requeridos." });
    }
    ...
    const fullPrompt = `${promptAI}\n\nAquí están los datos en formato JSON para tu análisis: ${JSON.stringify(
      datosParaGemini,
      null,
      2
    )}`;
    ...
    if (!analisisTexto) {
      ...
      analisisTexto =
        "No se pudo obtener el análisis. Error: El contenido de la IA fue bloqueado o está vacío.";
      console.error("❌ ERROR CRÍTICO: Respuesta de Gemini vacía o bloqueada.");
      console.error(
        "RESPUESTA COMPLETA DE GEMINI (DEBUG):",
        JSON.stringify(geminiResponse, null, 2)
      );
    }

    console.log(
      "RESPUESTA GEMINI (Extracción exitosa):",
      analisisTexto.substring(0, 100) + "..."
    );
